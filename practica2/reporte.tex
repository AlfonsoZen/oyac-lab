...
\begin{figure}[H]
  ...
  \caption{Módulo \texttt{div\_frec.vhd}, proporcionado por la profesora como base del
    proyecto.}
  \label{fig:div_frec}
\end{figure}
...
Como el manual pide explícitamente la implementación en VHDL, a continuación se incluye
el código equivalente de la máquina de estados, escrito directamente a partir de las
ecuaciones \ref{eq:d1}-\ref{eq:s0} obtenidas en la Parte I. Este código se presenta
únicamente como documentación de dicha implementación: no se integró al proyecto de
Quartus ni se compiló, por lo que no sustituye al esquemático de la figura
\ref{fig:diagrama-logico}, que es el que efectivamente se programó en la tarjeta.
...
begin
    ...
    -- Estado siguiente (ecuaciones 1 y 2)
    D1 <= (Q1 and not Q0) or (not Q1 and Q0 and X and Y);
    D0 <= (Q1 and not Q0) or (not Q0 and X and Y)
          or (not Q1 and Q0 and not X) or (not Q1 and Q0 and not Y);

    -- Flip-flops tipo D: capturan el estado siguiente en cada flanco de subida
    process (clk)
    begin
    ...
    -- Salidas (ecuaciones 3 y 4)
    S1 <= (not Q1 and not Q0) or (Q1 and Q0);
    S0 <= Q0;

end Behavioral;
\end{lstlisting}
\caption{Código VHDL de la máquina de estados, escrito a partir de las ecuaciones
  \ref{eq:d1}-\ref{eq:s0} de la Parte I. Entrada de \texttt{clk}: en la integración con
  \texttt{div\_frec.vhd}, correspondería a la señal \texttt{div\_clk} (reloj ya
  dividido), no al reloj de 50 MHz de la tarjeta.}
\label{fig:vhdl-maquina}
\end{figure}
...
El objetivo de la práctica se cumplió: a partir de la carta ASM llegamos a la tabla de
transiciones, redujimos por Karnaugh las ecuaciones de los flip-flops D y de las
salidas, y ese diseño es justo el que terminó compilado en Quartus y programado en la
tarjeta. Lo que más se me facilitó fue precisamente esa parte de pasar de la carta ASM a
la tabla, ya que al tener bien identificadas las rutas de decisión el llenado fue
prácticamente directo. En la tarjeta, para las combinaciones que sí alcanzamos a
fotografiar ($XY=00$, $01$ y $11$) el comportamiento de los LEDs coincidió con lo que la
tabla de la Parte I predecía, así que el diseño quedó validado, aunque nos falta
capturar la combinación $XY=10$ para dejar la comprobación completa. También queda
pendiente confirmar con la profesora si capturar la máquina como esquemático en vez de
escribirla en VHDL es válido para esta entrega, ya que el manual pide explícitamente esa
segunda opción.
